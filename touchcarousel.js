(function(f) {
    function r(a, c) {
        ...
        this._use3dTransforms = this._wasBlocked = this._blockClickEvents = !1;
        "ontouchstart" in window ? (this.hasTouch = !0, this._downEvent = "touchstart.rs", this._moveEvent = "touchmove.rs", this._upEvent = "touchend.rs", this._baseFriction = this.settings.baseTouchFriction) : (this.hasTouch = !1, this._baseFriction = this.settings.baseMouseFriction, this.settings.dragUsingMouse ? (this._downEvent = "mousedown.rs", this._moveEvent = "mousemove.rs", this._upEvent =
            "mouseup.rs", this._grabCursor, this._grabbingCursor, d = b._browser, d.msie || d.opera ? this._grabCursor = this._grabbingCursor = "move" : d.mozilla && (this._grabCursor = "-moz-grab", this._grabbingCursor = "-moz-grabbing"), this._setGrabCursor()) : this._itemsWrapper.addClass("auto-cursor"));
        this._prefix = "";
        if (this.hasTouch || this.settings.useWebkit3d)(d = document.createElement("div").style) && "perspective" in d ? this._use3dTransforms = !0 : d && "webkitPerspective" in d && (this._prefix = "-webkit-", this._use3dTransforms = !0);
        this._use3dTransforms ?
            (this._dragContainer.css(this._prefix + "transform-origin", "0 0"), this._dragContainer.css(this._prefix + "transform", "translateZ(0)"), this._xProp = this._prefix + "transform", this._xPref = "translate3d(", this._xSuf = "px, 0, 0)") : (this._xProp = "left", this._xPref = "", this._xSuf = "px");
        ...
        e.each(function(a) {
            ...
            k.index = a;
            k.posX = h;
            k.width = l.outerWidth(!0) || b.settings.itemFallbackWidth;
            h += k.width;
            if (this.hasTouch) {
                var c;
                l.find("a").each(function() {
                    c = f(this);
                    c.data("tc-href", c.attr("href"));
                    c.data("tc-target", c.attr("target"));
                    c.attr("href", "#");
                    c.bind("click", function(a) {
                        ...
                        c && "_self" !== c.toLowerCase() ? window.open(a) : window.location.href =
                            a
                    })
                })
            } else l.find("a").bind("click.touchcarousel", function(a) {
                if (b._successfullyDragged) return a.preventDefault(), !1
            });
            l.find(".non-draggable").bind(b._downEvent, function(a) {
        ...
        if (this.settings.pagingNav) {
            if (this._pagingEnabled = this.settings.snapToItems = !0, this._numPages = Math.ceil(this.numItems / this._itemsPerMove),
                this._currPageId = 0, this.settings.pagingNavControls) {
                ...
                for (g = 1; g <= this._numPages; g++) d = f('<a class="tc-paging-item" href="#">' + g + "</a>").data("tc-id", g), g === this._currPageId + 1 && d.addClass("current"), e.append(d);
        ...
        this.settings.directionNav && (this._itemsWrapper.after("<a href='#' class='arrow-holder left'><span class='arrow-icon left'></span></a> <a href='#' class='arrow-holder right'><span class='arrow-icon right'></span></a>"), this.arrowLeft = this.carouselRoot.find(".arrow-holder.left"), this.arrowRight = this.carouselRoot.find(".arrow-holder.right"),
            1 > this.arrowLeft.length || 1 > this.arrowRight.length ? this.settings.directionNav = !1 : this.settings.directionNavAutoHide && (this.arrowLeft.hide(), this.arrowRight.hide(), this.carouselRoot.one("mousemove.arrowshover", function() {
        ...
        this.settings.scrollbar ? (this._scrollbarHolder =
    ...
    r.prototype = {
        ...
        updateCarouselSize: function(a) {
            this.carouselWidth =
                this.carouselRoot.width();
            if (this.settings.scrollToLast) {
                var c = 0;
                if (this._pagingEnabled) {
                    var b = this.numItems % this._itemsPerMove;
                    if (0 < b)
                        for (b = this.numItems - b; b < this.numItems; b++) c += this.items[b].width;
            ...
            this.settings.scrollbar && (c = Math.round(this._scrollbarHolder.width() / (this._maxXPos / this.carouselWidth)), this.scrollbarJQ.css("width", c),
                this._scrollbarDist = this._scrollbarHolder.width() - c);
            if (!this.settings.scrollToLast) {
                ...
                this._wasBlocked && (this._arrowLeftBlocked = this._arrowRightBlocked = this._wasBlocked = !1, this.arrowRight.removeClass("disabled"), this.arrowLeft.removeClass("disabled"))
            }
            a || (a = this.endPos =
                this._getXPos(), 0 < a ? a = 0 : a < this.carouselWidth - this._maxXPos && (a = this.carouselWidth - this._maxXPos), this.animateTo(a, 300, "easeInOutSine"))
        },
        animateTo: function(a, c, b, e, d, g, k) {
            function l() {
                h._isAnimating = !1;
                h._releaseAutoplay();
            ...
            this._decelerationAnim = f(r).animate(t, {
                ...
                step: function() {
                    ...
                    u[n] =
                        p + Math.round(this.containerPos) + q
                },
                complete: function() {
                    e ? h._decelerationAnim = f(t).animate(x, {
                        ...
                        complete: function() {
                            m && (v[n] = p + Math.round(-x.containerPos / w * h._scrollbarDist) + q);
                            u[n] = p + Math.round(x.containerPos) + q;
                            l()
                        }
                    }) : (m && (v[n] = p + Math.round(-t.containerPos / w * h._scrollbarDist) + q), u[n] = p + Math.round(t.containerPos) + q, l())
                }
            })
        },
        destroy: function() {
        ...
        _getPageIdFromItemId: function(a) {
            for (var c = this._itemsPerMove,
                    b = 0; b < this._numPages; b++)
        ...
        _disableLeftArrow: function() {
            this._arrowLeftBlocked || this.settings.loopItems || (this._arrowLeftBlocked = !0, this.arrowLeft.addClass("disabled"), this._arrowRightBlocked &&
                (this._arrowRightBlocked = !1, this.arrowRight.removeClass("disabled")))
        },
        ...
        _getItemAtPos: function(a) {
            ...
            for (var c, b = 0; b < this.numItems; b++)
                if (c = this.items[b], a >= c.posX && a < c.posX + c.width) return c;
            return -1
        },
        _releaseAutoplay: function() {
            this.autoplay && this.wasAutoplayRunning &&
                (this._isHovering || this._resumeAutoplay(), this.wasAutoplayRunning = !1)
        },
        ...
        _getXPos: function(a) {
            ...
            return this._use3dTransforms ? (a = a.css(this._prefix + "transform").replace(/^matrix\(/i, "").split(/, |\)$/g), parseInt(a[4],
                10)) : Math.round(a.position().left)
        },
        _onDragStart: function(a) {
            if (!this._isDragging) {
                ...
                var c;
                if (this.hasTouch)
                    if (this._lockYAxis = !1, (c = a.originalEvent.touches) && 0 < c.length) c = c[0];
                    else return !1;
                ...
                this._use3dTransforms && (this._dragContainer.css(this._prefix + "transition-duration",
                    "0"), this._dragContainer.css(this._prefix + "transition-property", "none"));
        ...
        _onDragMove: function(a) {
            ...
            if (this.hasTouch) {
                ...
                b = b[0];
                if (Math.abs(b.clientY - this._startMouseY) > Math.abs(b.clientX - this._startMouseX) + 3) return this.settings.lockAxis && (this._lockYAxis = !0), !1
            } else b = a;
            a.preventDefault();
            ...
            if (0 != a) {
                ...
                0 <= e ? (a /= 4, this._disableLeftArrow()) : e <= this.carouselWidth - this._maxXPos ?
        ...
        _onDragRelease: function(a) {
            if (this._isDragging) {
                ...
                if (this.settings.snapToItems) {
                    this.autoplay && this.settings.autoplayStopAtAction &&
                        this.stopAutoplay();
        ...
        _setGrabbingCursor: function() {
            this._grabbingCursor ? this._itemsWrapper.css("cursor", this._grabbingCursor) : (this._itemsWrapper.removeClass("grab-cursor"),
